...
description: 根据氢能搜索结果JSON文件生成月度行业动态报告。Use when user asks to generate 氢能月报, monthly hydrogen report, create report from JSON search results, or write 双碳新能源领域月度行业动态. Reads JSON files to extract URLs and key_info, outputs both MD and Word formats following the template structure. Enforces URL integrity, key_info injection, minimum item counts, and fixed subcategory names.
---
...
```
⚠️ 以下11个子分类名称和顺序为固定模板，子代理不得自行调整、合并或重命名：
...
| （三）项目建设 | dimension=project, sub_category∈{construction, groundbreaking, building}（仅覆盖开工/建设阶段） |
| （四）项目投产 | dimension=project, sub_category∈{commissioning, operation, delivery} |
| （五）项目测试和验收 | dimension=project, sub_category∈{testing, acceptance, validation, milestone} |
| （六）市场和销售 | dimension=industry, sub_category∈{supply_chain, market_data, enterprise} |
| （七）合作投资重组 | dimension=industry, sub_category∈{exchange_announcement, soe_bidding, financial_media} |
| （八）海外项目 | dimension=overseas, sub_category∈{global_policy, chinese_overseas, overseas_technology} |
| （九）国内政策风向 | dimension=policy, sub_category∈{national_policy, local_policy, policy_interpretation, policy_impact} |
| （十）市场和成本数据 | dimension=industry, sub_category=market_data + dimension=technology 成本相关 + dimension=project 投资规模 |
| （十一）趋势研判 | 基于所有数据的编辑分析（**必须使用独立来源URL**） |
...
- 研发主体（如"中科院大连化物所"）
- 技术路线（如"PEM电解槽"）
- 关键技术指标（如"电流密度3A/cm²"、"能耗4.3kWh/Nm³"）（**要求≥3个参数**，如功率+电流密度+能耗+寿命等，至少包含3个）
- 研究阶段（如"中试示范"）

**项目维度**：
- 项目全称
- 所在地（如"内蒙古鄂尔多斯"）
- 投资金额（**要求精确到万元**，如"总投资9493.19万元"，仅在JSON中确实只有概数时才使用"亿元"）
- 产能规模（**要求包含单位**，如"年产绿氨30万吨"、"制氢规模22000Nm³/h"）
- 参与方（至少2个）
...
[第1段：项目全称+测试/验收时间+测试内容]
[第2段：测试结果+关键指标+验收结论+参与方]
...
- [ ] 重要信息精确3条
- [ ] 趋势研判精确5条
- [ ] 每个子分类按新标准检查（研发≥4条、项目前期≥5条、项目建设≥3条、项目投产≥4条、项目测试和验收≥4条、市场和销售≥4条、合作投资重组≥3条、海外≥5条、国内政策≥6条、市场和成本数据≥4条）
