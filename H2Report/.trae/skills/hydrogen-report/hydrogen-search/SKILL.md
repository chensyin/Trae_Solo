...
所有搜索必须使用 **searxng MCP** 工具执行，调用方式：

```
run_mcp(server_name="mcp_searxng", tool_name="searxng_web_search", args={
  "query": "搜索语句",
  "language": "zh" 或 "en",
  "time_range": "month"（可选，限定当月范围）,
  "pageno": 1（可选，翻页）
})
```

**参数说明**：
- `query`：搜索语句，使用各维度Skill中定义的搜索模板
- `language`：中文搜索用"zh"，英文搜索用"en"，默认"all"
- `time_range`：可选值"day"/"month"/"year"，建议中文搜索使用"month"限定当月
- `pageno`：页码，默认1，如需更多结果可翻页

**优势**：searxng聚合多个搜索引擎（Google、Bing、百度等），中文搜索结果更丰富，且不受单一搜索引擎限制。
...
| # | 搜索语句 | searxng参数 | 目的 |
|---|---------|-------------|------|
| B1 | `氢能 (政策 OR 项目 OR 技术 OR 投资 OR 签约 OR 投产) [月份] [年份]` | language="zh", pageno=1 | 获取当月氢能领域综合动态，识别热点事件 |
| B2 | `氢能 (展会 OR 论坛 OR 签约 OR 开工 OR 投产 OR 突破) [月份] [年份]` | language="zh", pageno=1 | 聚焦当月重大事件（展会、签约、投产等） |
| B3 | `hydrogen energy (policy OR project OR investment OR breakthrough) [月份英文] [年份]` | language="en", pageno=1 | 获取国际氢能动态，识别海外热点 |
