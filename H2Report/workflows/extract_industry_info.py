...
def extract_project_info(items):
    ...
    for item in items:
        ...
        if any(word in title+summary for word in ['建设', '建造', '施工', '安装', '工程']):
            stages.append('建设')
        if any(word in title+summary for word in ['验收', '运行', '运营', '投产', '投产', '完成', '成功', '稳定运行', '验证', '测试']):
            stages.append('验收/运行')
        if stages:
            projects.append({
                'item': item,
...
def extract_company_dynamics(items):
    ...
    for item in items:
        ...
        if any(word in title+summary for word in ['突破', '首创', '首次', '领先', '领先']):
...
def main():
    ...
    for proj in projects:
        item = proj['item']
        print(f"  - ID:{item['id']} 阶段:{proj['stages']} {item['title'][:50]}...")
    
    print(f"\n企业动态 ({len(company_dynamics)}条):")
    for item, dyn_type in company_dynamics:
        print(f"  - ID:{item['id']} 类型:{dyn_type} {item['title'][:50]}...")
...
def generate_markdown(categorized, projects, company_dynamics):
    ...
    # 提取企业名称（简单从标题或摘要中提取）
    companies = set()
    for item in sum(categorized.values(), []):
        # 简单提取，实际可更复杂
        text = item['title'] + item['summary']
        # 已知一些企业名称
        known_companies = ['稳石氢能', '国富氢能', '昊臻氢能', '航天工程', '东方电气', '中石化', '深圳大学', '四川大学', '青岛炼化', '中信证券', '中国航发', '上海外高桥']
